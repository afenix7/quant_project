...
def generate_sample_data(symbol='000001', days=252):
    """
    生成示例数据（用于测试）
    """
    import numpy as np
    
    print(f"生成示例数据: {symbol}")
    ...
    open_prices = close + np.random.randn(days) * 0.5
    high = np.maximum(open_prices, close) + np.random.rand(days) * 1
    low = np.minimum(open_prices, close) - np.random.rand(days) * 1
    ...
    data_file = os.path.join(data_dir, f'{symbol}_sample.csv')
